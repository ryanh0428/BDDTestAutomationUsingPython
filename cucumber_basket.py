class CucumberBasket:
    def __init__(self, initial_count = 0, max_count=10):
        ...
        #underscore is to indicate the variable is for interanl use only
    ...
    def add(self, count =1):
        ...
        if new_count > self.max_count:
            raise ValueError("Attempted to add too many cucumber")
    ...
    def remove(self,count = 1):
        new_count = self.count - count
        if new_count < 0:
            raise ValueError("Attempted to remove too many cucumber")
        self._count = new_count
