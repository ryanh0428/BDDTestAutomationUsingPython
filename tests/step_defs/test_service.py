...
from pytest_bdd import scenarios, parsers, given, then, parsers
...
@given(parsers.parse('the DuckDuckGo API is queried with {phrase}'),target_fixture="ddg_response", converters=dict(phrase=str))
#@given('the DuckDuc kGo API is queried with "<phrase>"',converters=dict(phrase=str)) #only the given can be used as fixture
...
@then(parsers.parse('the response contains result for {phrase}'))
